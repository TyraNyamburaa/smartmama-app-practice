...
def invite_new_admin(db: Session, data: UserSignup) -> str:
    ...
    try:
        send_invite_email(email, data.first_name, invite_url, "admin")
...
def register_admin(db: Session, data: UserSignup) -> Admin:
    email = str(data.email).lower()
    existing_user = user_repository.get_user_by_email(db, email)
    if existing_user:
    ...
    admin_count = admin_repository.count_admins(db)
    is_superadmin = admin_count == 0

    profile = admin_repository.create_admin_profile(db, new_user.user_id, is_superadmin=is_superadmin)

    audit_log_repository.create(
        db,
        actor_id=new_user.user_id,
        event_category="action",
        action_type="admin_self_registered",
        target_id=profile.admin_id,
        success=True,
        details="First super admin registered" if is_superadmin else "Admin registered",
    )
...
def update_admin_profile(db: Session, admin: Admin, data: AdminUpdateProfile) -> Admin:
...
def deactivate_admin(db: Session, admin: Admin, super_admin_user_id: UUID) -> Admin:
